...
export class HouseView extends Component {
    ...
    render() {
        console.log("Acolo", this.props);
        return (
            <div className="col-1-2">
                <article className="post-wrap">
                    <div className="post">
                        <h2 className="entry-title"><a href="#0">{this.props.house.name}</a></h2>
                        <Button
                            className="btn read-more"
                            onClick={() => this.props.onClick && this.props.onClick(this.props.house.id)}
                            style={{ backgroundColor: "green", color: "white" }}
                        >
                            Vizualizeaza facturi
                        </Button>
                        {/*<Button*/}
                        {/*    onClick={() => this.onDelete(this.props.house.id)}*/}
                        {/*    style={{ margin: "20px", backgroundColor: "red", color: "white" }}*/}
                        {/*>*/}
                        {/*    Sterge*/}
                        {/*</Button>*/}
                        <Button
                            onClick={() => this.onEdit(this.props.house.id)}
